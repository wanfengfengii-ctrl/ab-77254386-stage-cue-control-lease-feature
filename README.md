...
1. pytest：针对同一套 PostgreSQL 验证**数据库事务竞争**（24 路并发争抢仅 1 个
   ...
   **场次归属**（单动作与联动事件在原事务内计入当前场次、并发开始仅一个
   ...
   不再显示、重新申请 / 释放 / 单动作 / 联动均不覆盖旧记录）、**执行历史
   游标分页**（联动两事件以本组最小事件编号归组、编号被别的动作插入也
   不分离、分组不跨页；以不可变事件编号为游标逐页翻完全部记录且
   新执行插入中途不重复不跳过；异常与确认随所属事件返回；非法游标
   `history_cursor_invalid` 且不影响已加载页）；
2. Vitest：倒计时边界、接管 / 旧令牌界面逻辑，场次全流程组件测试
   （开始 → 两类动作执行 → 结束后摘要冻结），以及异常报告 / 确认组件测试
   （提交成功后轮询展示报告人与时间、空白说明就地反馈且不发请求、同一页面
   改席名不出现确认入口、新事件切换卡片后不误显旧异常），执行历史组件
   测试（最新页在前、联动相邻归组（编号被插入也不分离）、异常与确认随
   事件、后续页失败 / 游标非法时保留已加载内容并在尾部重试、关闭重开
   回到最新页）；
3. Playwright：两个真实浏览器上下文模拟双控制席——同时争抢只有一人持有、
   ...
   刷新后仍展示已确认结果与确认席位），以及执行历史用例（单动作 +
   联动执行、异常报告并由另一席确认后，打开历史逐页核对：联动相邻归组、
   异常与确认随事件、逐页无重复无缺口，刷新后重新打开记录一致）。
...
负责人复盘时需要的不再只是每张卡片的最近结果，而是**按发生顺序追查每一次
动作执行**。控制台头部提供「执行历史」入口，打开后先展示**最新一页**，
可继续「加载更早记录」：

- 历史完全复用现有数据：`action_events`（动作事件、`link_id` 联动标识、
  `session_id` 场次归属）与 `action_anomalies`（异常与确认）。服务端
  **只读**接口 `GET /api/history` 以**一条 SQL**（JOIN 动作、场次、异常，
  联动分组走 `action_events(link_id)` 索引）取回全部关联信息，绝不逐条
  补查；按事件编号倒序返回**执行时间、动作（编号与名称）、执行席位、
  所属场次**，`link_id` 不为空即联动事件，异常内容与确认结果随所属事件
  一起返回；
- **同一次联动的两条事件在界面相邻归组**：联动事件以「本组最小事件编号」
  为排序锚点，即便全局序列把另一动作的编号发在两条联动事件之间，界面仍把
  这一对渲染在同一个「联动执行」分组内、互不分离；联动分组也**不会被
  页边界切开**（越界成员的配对事件随本页一起返回）；
- 翻页采用以**不可变事件编号**为游标的键集分页：响应给出
  `next_cursor`（本页最早一条的编号；联动分组即本组锚点编号），下一页
  请求 `sort_key < cursor`。游标一旦发放即固定，**翻页期间即使产生新的
  执行也不会重复或跳过旧记录**（新记录只属于重新打开时的更新首页）；
- 游标非法（非正整数等）返回可识别的 `400 history_cursor_invalid`，
  **页面保留已加载内容**，仅在列表尾部提示；后续页请求失败（网络错误等）
  同样在**列表尾部提示重试，不清空当前结果**，重试成功后旧页追加、既不
  重复也无缺口；
- 既有动作快照、租约执行、联动、场次与异常接口字段**全部保持可用**，
  `/api/history` 与索引均为新增。刷新页面后重新打开历史，最新一页记录
  与刷新前一致。
...
| GET | `/api/history` | 只读执行历史（`?cursor=事件编号&limit=20`），最新一页在前，响应给出 `next_cursor`；联动两条相邻、异常与确认随事件返回 |
...
`400 history_cursor_invalid`（执行历史翻页游标非法，可识别且不影响
已加载内容）。
...
backend/            FastAPI + psycopg + PostgreSQL
  ...
  app/history.py      只读执行历史（事件编号游标分页 / 联动相邻归组）
...
frontend/           React + Vite + TypeScript
  ...
  src/App.tsx         短轮询控制台（含场次控制、本轮摘要、卡片异常留痕与执行历史弹层）
  test/               Vitest 单元 + 组件测试（mock 仅存在于测试目录）
  e2e/                Playwright 双浏览器交接 + 联动 + 场次 + 异常确认 + 执行历史用例（真实服务）
