...
1. pytest：针对同一套 PostgreSQL 验证**数据库事务竞争**（24 路并发争抢仅 1 个
   ...
   **场次归属**（单动作与联动事件在原事务内计入当前场次、并发开始仅一个
   ...
   不再显示、重新申请 / 释放 / 单动作 / 联动均不覆盖旧记录）与**执行历史**
   （事件编号倒序返回执行时间 / 动作 / 席位 / 场次、联动两条相邻归组且
   另一动作编号夹在中间也不拆散、异常与确认结果随事件带出、不可变游标
   跨页无重复无遗漏且翻页期间的新执行不扰动旧页、游标非法为
   history_cursor_invalid、整页一条集合化 SQL 不逐条补查）；
2. Vitest：倒计时边界、接管 / 旧令牌界面逻辑，场次全流程组件测试
   （开始 → 两类动作执行 → 结束后摘要冻结），异常报告 / 确认组件测试
   （提交成功后轮询展示报告人与时间、空白说明就地反馈且不发请求、同一页面
   改席名不出现确认入口、新事件切换卡片后不误显旧异常），以及执行历史
   组件测试（最新一页、单动作与联动归组、异常随事件展示、逐页加载、尾部
   失败可重试且不清空、非法游标保留已加载内容、刷新后记录一致）；
3. Playwright：两个真实浏览器上下文模拟双控制席——同时争抢只有一人持有、
   ...
   联动各执行一次 → 结束场次 → 摘要固定且随后执行不再计入），异常
   交接用例（一席执行并报告异常、另一浏览器看到待确认记录后确认、两侧
   刷新后仍展示已确认结果与确认席位），以及执行历史复盘用例（执行单动作
   并报告 / 确认异常、再做一次联动、打开历史逐页核对联动归组与异常归属、
   翻页期间新执行不改变已加载记录、刷新后重新打开仍得到一致记录）。
...
演出复盘时负责人要按动作**实际发生顺序**追查执行，而不是只能看到每张卡片
的最近结果。控制台页头提供**执行历史**入口（只读弹窗，未填席名也可打开）：

- 打开后先展示**最新一页**，页底「加载更早记录」按不可变事件编号游标继续
  向前翻；每次响应给出 `next_cursor` 与 `has_more`。历史复用动作事件、
  场次、联动标识与异常记录，按**事件编号倒序**返回执行时间、动作（编号与
  名称）、席位与场次；**同一次联动的两条事件在界面相邻归为一组**，异常内容
  与确认结果随所属事件展示；
- 接口为**只读** `GET /api/history`，默认每页 20 个执行组（联动两条算一组，
  可用 `limit` 覆盖、上限 100）。整页由**一条集合化 SQL** 返回——场次与
  异常通过 JOIN 带出、联动锚点用一次 `GROUP BY` 聚合算出，**不逐条补查**
  关联信息；
- **翻页稳定**：游标是只追加、不可变的事件编号。翻页期间即使产生新执行，
  新事件编号只会更大，永远不会出现在更早的页上——已加载记录既不重复也不
  跳过。联动两条事件以「该组最大事件编号」为统一排序锚点，即便另一动作的
  事件编号理论上夹在两条联动插入之间，联动组仍相邻完整；`锚点 < cursor`
  让下一页从整组之后开始，不会只重放半组；
- 游标非法（空白、非数字、0、负数或超出 bigint）返回可识别业务错误
  `400 history_cursor_invalid`；界面**保留已加载内容**，仅在列表尾部说明并
  提供「重新打开最新一页」。后续页请求失败（网络/5xx）同样只在列表尾部
  提示「重试」，**不清空当前结果**；
- 动作快照、租约执行、联动、场次与异常接口的原有字段全部保持可用，历史
  接口与字段均为新增。响应形状：`{"items":[{"link_id":…,
  "events":[{"event_id","occurred_at","action_id","action_label","holder",
  "result","link_id","session":{"id","name","status"},"anomaly":…}]}],
...
| GET | `/api/history?cursor=&limit=` | 只读执行历史（执行历史），事件编号倒序、联动归组、游标翻页 |
...
`400 history_cursor_invalid`（执行历史分页游标非法，界面保留已加载内容）。
...
backend/            FastAPI + psycopg + PostgreSQL
  ...
  app/history.py      只读执行历史（不可变事件编号游标 / 联动归组 / 一条集合化 SQL）
  app/db.py           连接池、建表、状态快照
  tests/              pytest：事务竞争、过期边界、HTTP 端到端、场次归属、异常留痕、执行历史
frontend/           React + Vite + TypeScript
  src/api.ts          类型化 HTTP 客户端（真实接口）
  src/lease.ts        服务端时钟锚定的倒计时（纯函数，单测覆盖边界）
  src/App.tsx         短轮询控制台（含场次控制、本轮摘要与卡片异常留痕）
  src/HistoryModal.tsx 执行历史弹窗（最新页 → 20 组/页逐页加载更早记录；尾部失败只提示重试）
  test/               Vitest 单元 + 组件测试（mock 仅存在于测试目录）
  e2e/                Playwright 交接 + 联动 + 场次 + 异常确认 + 执行历史用例（真实服务）
